...
class DPIACLAnswerSheet extends Component {
  constructor(props) {
    super(props);
    this.state = {
      navBar: 'nav-md',
  ...
  changeQuestion(e, rid) {
    var id = rid + 1;
    for (var i = 0; i < this.state.allQuestion.length; i++) {
      if (id <= (this.state.allQuestion.length - 1) && this.state.allQuestion[id].action_status !== 'open') {
        this.setState({
  ...
  changeQuestionPrev(e, rid) {
    ...
    for (var i = 0; i < this.state.allQuestion.length; i++) {
      if (id >= 0 && this.state.allQuestion[id].action_status !== 'open') {
        this.setState({
          quesNo: this.state.allQuestion[id].id,
          risk_weightage: this.state.allQuestion[id].risk_weightage_status,
          answerToUpload: {
  ...
  getAllSecQuestion(data) {
    ...
    if (data.status === 200) {
      for (var i = 0; i < data.data.length; i++) {
        if (data.data[i].action_status != 'open') {
          arrayTopush.push(data.data[i])
        }
      }
      this.setState({
        allQuestion: arrayTopush,
        quesNo: arrayTopush[0].id,
        risk_weightage: arrayTopush[0].risk_weightage_status
      });
      this.props.questionAction.getAnswerByQuesId(arrayTopush[0].id, this.gotAnswer);
      this.props.questionAction.getAttachment(arrayTopush[0].id, this.afterGetAllFile);
      this.props.questionAction.getNotes(arrayTopush[0].id, this.getAllNotes);
    }
  }
                  ...
                  <div className="row">
                    <div className="col-md-12 col-sm-12 col-xs-12">
                      <div className="x_panel">
                        <div class="x_title">
                          <h2> <a>Assessment</a> &gt; Assessment 1  </h2>
                          <input class="star" type="checkbox" title="Mark complete" />
                                    ...
                                    <div className="tab-pane active" id="list-of-questions">
                                      ...
                                      <div className="row">
                                        <div className="col-md-12 form-inline felement">
                                          <button type="button" className="btn btn-primary" data-toggle="modal" data-target=".delegate-modal" onClick={(e) => this.clickDelegate(e,ques)} >Delegate</button>
                                          <input type="submit" className="btn btn-primary" onClick={(e) => this.requestSignOff(e, ques)} name="submitbutton" id="id_submitbutton" value="Mark As Completed" />
...
export default connect(mapStateToProps, mapDispatchToProps)(DPIACLAnswerSheet);